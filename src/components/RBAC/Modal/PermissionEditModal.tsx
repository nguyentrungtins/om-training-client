...
import { EditPermissionFormType } from "@/types/form/FormType.interface";
import { sendRequest } from "@/api/swr/editPermission";
import { useSelector } from "react-redux";
import { selectTokenState } from "@/redux/tokenSlice";
import { RolePermission } from "../types/Permission.interface";
...
const openNotification = (type: string) => {
  if (type === "success") {
    ...
    notification.error({
      message: "Update role permissions failed!",
      description: "Some Error Occur when updating role permissions!",
      onClick: () => {
        console.log("Notification Clicked!");
      },
    });
...
const PermissionEditModal: NextPage<PageProps> = ({ data }) => {
  // const { update } = useSession();
  const token = useSelector(selectTokenState);
  const [open, setOpen] = useState<boolean>();
  const [form, setForm] = useState<EditPermissionFormType>({
    name: data.name,
  ...
  const { trigger, isMutating } = useSWRMutation(
    `${prefixURL}/user/update-role-permissions`,
    sendRequest
  );
  ...
  const RoleSelectOptions = [
    ...
    { value: PERMISSION.POST },
    { value: PERMISSION.PUT },
    { value: PERMISSION.PATCH },
            ...
            <div className="relative bg-white rounded-lg shadow ">
              ...
              <div className="p-6 space-y-6">
                ...
                >
                  ...
                  <Form.Item<FieldType> label="Permissions" name="permissions">
                    <Select
                      mode="multiple"
                      style={{ width: "100%" }}
